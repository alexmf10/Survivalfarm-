...
func calculate_damage(base_damage: float, armor: float = 0.0) -> float:
	var net_damage: float = maxf(base_damage - armor, MIN_DAMAGE)
	return net_damage


## Variante con multiplicador crítico.
## critical_multiplier = 1.0 para golpe normal, 2.0 para crítico, etc.
func calculate_critical_damage(base_damage: float, armor: float, critical_multiplier: float) -> float:
	var raw: float = base_damage * critical_multiplier
	return maxf(raw - armor, MIN_DAMAGE)
